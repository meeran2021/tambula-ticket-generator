...
logging.basicConfig(filename='app.log', level=logging.ERROR)  # Generate logs seperately
...
def generate_tickets():
    try:
        if request.method == 'POST':
            num_of_sets = int(request.json.get('num_of_sets'))
            if num_of_sets <= 0:
                raise ValueError("Number of tickets must be greater than zero.")
        
        generated_tickets = generate_unique_tickets(num_of_sets)

        save_tickets_to_db(generated_tickets)

        return jsonify({'message': f'Successfully generated and saved {num_of_sets} tickets .'}), 200
...
def show_tickets():
    try:
        if request.method == 'GET':   
            ...
            tickets_per_page = int(request.args.get('tickets_per_page', 30))
